...
@Component({ selector: 'app-product-form-dialog', standalone: true, templateUrl: './product-form-dialog.component.html', styleUrls: ['./product-form-dialog.component.scss'],
...
export class ProductFormDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.form = this.fb.group({ name: [p?.name || '', [Validators.required]], description: [p?.description || ''], categoryId: [p?.category?.id || null], price: [p?.price || null], discount: [p?.discount || null], image: [p?.image || ''], isActive: [p?.isActive ?? 1] });
  }
